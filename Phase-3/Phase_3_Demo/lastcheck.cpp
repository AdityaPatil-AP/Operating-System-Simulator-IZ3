...
void cpu::OS()
{
    ...
    int var = PCBlist[currentPCBIndex].o_track;
    int m = 0;
    for (int i = var; i < (var + 10); i++)
    {
    ...
    ss.ofbq.push(buffer);
    (PCBlist[currentPCBIndex]).no_ol--;

    PCBlist[currentPCBIndex].o_track += 10;
    return;
}
...
void cpu::RD(int currentPCBIndex)
{
    ...
    int addr = (readPTrow((PTrow - 1)));

    memcpy(m[addr], drum.aux[track], 40);
    if (m[addr][0] == '$' && m[addr][1] == 'E' && m[addr][2] == 'N' && m[addr][3] == 'D')
    {
        terminate({1});
        // terminate if no data card present
    }
    (PCBlist[currentPCBIndex].d_track) += 10;
    (PCBlist[currentPCBIndex].no_dc)--;
...
void cpu ::write(int address)
{
    ...
    PCBlist[currentPCBIndex].o_track = auxRow;
    PCBlist[currentPCBIndex].no_ol++;
    auxRow += 10;
    memcpy(drum.aux[PCBlist[currentPCBIndex].o_track], buffer1.c_str(), 40);
    // outFile
    //     << buffer1 << endl;
...
void cpu ::terminate(vector<int> em)
{
    ...
    if (em[0] == 0)
    {
        ...
        string buff1 = "Normal Termination";
        PCBlist[currentPCBIndex].o_track = auxRow;
        PCBlist[currentPCBIndex].no_ol++;
        auxRow += 10;
        memcpy(drum.aux[PCBlist[currentPCBIndex].o_track], buff1.c_str(), 40);
        // 	hRead = true;
    }
    else
    {
        ...
        string buff1 = "Abnormal Termination";
        PCBlist[currentPCBIndex].o_track = auxRow;
        PCBlist[currentPCBIndex].no_ol++;
        auxRow += 10;
        memcpy(drum.aux[PCBlist[currentPCBIndex].o_track], buff1.c_str(), 40);
        for (int i = 0; i < em.size(); i++)
        {
            ...
            string buff1 = errorMessage[em[i]];
            PCBlist[currentPCBIndex].o_track = auxRow;
            PCBlist[currentPCBIndex].no_ol++;
            auxRow += 10;
            memcpy(drum.aux[PCBlist[currentPCBIndex].o_track], buff1.c_str(), 40);
...
void cpu::simulation()
{
    ...
    masterMode(0);
    cout << "Inside simulation IOI : " << IOI << endl;
}
